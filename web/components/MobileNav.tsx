  ...
  return (
    <div className="sm:hidden" suppressHydrationWarning>
      <button
        ...
        className="inline-flex items-center gap-2 px-3 py-2 border rounded text-sm hover:bg-black/5"
      >
        <span aria-hidden>☰</span>
        <span>{t('mobile.menu', 'Menu')}</span>
      </button>
      ...
      <div className={`fixed inset-0 z-40 transition-opacity ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        />
        <div className={`absolute right-0 top-0 h-full w-[85%] max-w-[360px] bg-white dark:bg-neutral-900 shadow-xl z-50 border-l transform transition-transform duration-300 ease-out ${open ? 'translate-x-0' : 'translate-x-full'}`}>
